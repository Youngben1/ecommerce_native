...
export async function createProduct(req, res) {
	try {
		...
		if (!req.files || req.files.length === 0) {
			return res.status(400).json({ message: "At least an image is required" });
		}
		...
		const imageUrls = uploadResults.map((result) => result.secure.url);
...
export async function updateProduct(_, res) {
	try {
		const { id } = req.params;
