...
export async function updateCartItem(req, res) {
	try {
        ...
        if(quantity < 1) {
            return res.status(400).json({error:"Quantitymust be atleast 1"});
        }
...
export async function removeFromCart(req, res) {
	try {
        ...
        await cart.save();
        res.status(200).json({error:"Item removed from cart successfully", cart});
...
export async function clearCart(req, res) {
	try {
        ...
        cart.items = [];
        await cart.save();
        res.status(200).json({error:"Cart cleared", cart});
	} catch (error) {
