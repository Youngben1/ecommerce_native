    ...
    <SafeScreen>
      ...
      <ScrollView className='flex-1' contentContainerStyle={{paddingBottom: 100}} showsVerticalScrollIndicator={false}>
        ...
        <View className='px-6 pt-6'>
          <Text className="text-text-primary text-lg font-bold mb-4">Security Settings</Text>
          {securitySettings.map((setting) => (
            <TouchableOpacity key={setting.id} className='bg-surface rounded-2xl p-4 mb-3' activeOpacity={setting.type === "toggle" ? 1: 0.7} >
              <View className="flex-row items-center">
                ...
                <View>
                  <Text className="text-text-primary font-semibold text-base">{setting.title}</Text>
          ...
          {privacySettings.map((setting) => (
            <View key={setting.id}>
              <View className="bg-surface rounded-2xl p-4 mb-3">
                <View className='flex-row items-center'>
                  <View className="bg-primary/20 rounded-full w-12 h-12 items-center justify-center mr-4">
                    <Ionicons name={setting.icon as any} siz={24} color="#1DB954" />
                  </View>
        ...
        {/* ACCOUNT SETTINGS */}
        <View className='px-6 pt-4'>
          <Text className="text-text-primary text-lg font-bold mb-4 px-6">Account Settings</Text>
        ...
        <View className="px-6 pt-6 pb-4">
          <View className="bg-primary/10 rounded-2xl p-4 flex-row">
            ...
            <Text className="text-text-secondary text-sm ml-3 flex-1">
              We do not take your privacy seriously. Your data is encrypted and stored securely. You can
              manage your privacy settings at any time, all na whining sha but no worry.
            </Text>
          </View>
        </View>
      </ScrollView>
