    ...
    <SafeScreen>
      <ScrollView className='flex-1' contentContainerStyle={{paddingBottom: 100}} showsVerticalScrollIndicator={false}>
        ...
        <View className='flex-row flex-wrap gap-2 mx-6 mb-3'>
          {MENU_ITEMS.map((item) => (
            <TouchableOpacity key={item.id} className='bg-surface rounded-2xl p-6 items-center justify-center' style={{width: "48%"}} onPress={() => handleMenuPress(item.action)} activeOpacity={0.7}>
              <View className='rounded-full w-16 h-16 mb-4 items-center justify-center' style={{backgroundColor: item.color + "20"}}>
                <Ionicons name={item.icon} size={28} color={item.color} />
              </View>
              <Text className='font-bold text-text-primary text-base'>{item.title}</Text>
            </TouchableOpacity>
          ))}
        </View>
        ...
        <View className='mb-3 mx-6 bg-surface rounded-2xl p-4'>
          <TouchableOpacity activeOpacity={0.7} className='flex-row items-center justify-between py-2'>
            <View className='flex-row items-center'>
